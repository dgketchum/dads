...
def process_tile(
    tile, points, raster_dir, out_dir, index_col="fid", overwrite=False, bad_tiles=None
):
    ...
    if all([s in complete_tile_stations for s in tile_points.index]) and not overwrite:
        ...
        tile_points = tile_points.loc[
            [i for i in tile_points.index if i not in complete_tile_stations]
        ]
    ...
    first, bad_file_ct, values = True, False, None
    ...
    for day in range(1, 366):
        ...
        try:
            values = zonal_stats(
                tile_points, raster_file, stats="mean", geojson_out=True
            )
...
def extract_raster_values_by_tile(
    shapefile_path,
    raster_dir,
    table_out,
    shuffle=False,
...
if __name__ == "__main__":
    ...
    shapefile_path_ = os.path.join(
        root, "climate", "ndbc", "ndbc_meta", "ndbc_stations.shp"
    )
    extract_raster_values_by_tile(
        shapefile_path_,
        raster_dir_,
        solrad_out,
        num_workers=16,
