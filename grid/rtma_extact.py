...
def extract_rtma(
    ...
    if "END" in station_list.columns:
        station_list["end_dt"] = [
            pd.to_datetime(r["END"]) for i, r in station_list.iterrows()
        ]
        station_list = station_list[
            station_list["end_dt"] > pd.to_datetime("2016-01-01")
        ]
...
def proc_time_slice(indexer_, date_string_, grb_, nc_, out_, overwrite_, model_):
    ...
    grid_points_radians = np.deg2rad(
        np.stack(
            (ds["latitude"].values.ravel(), ds["longitude"].values.ravel()), axis=-1
        )
    )
    station_points_radians = np.deg2rad(
        np.stack((indexer_["slat"].values, indexer_["slon"].values), axis=-1)
    )
    ...
    for fid in fids_:
        try:
            ...
            _file = os.path.join(
                dst_dir, "{}_{}.parquet".format(fid, date_string_.replace("-", ""))
            )
...
def get_grb_files(date_str, model, dst, nc_file, max_threads=6):
    ...
    FH = FastHerbie(
        ...
        **{"save_dir": dst, "priority": ["aws", "nomads"]},
    )
    ...
    ds_["longitude"] = ds_["longitude"] - 360.0
