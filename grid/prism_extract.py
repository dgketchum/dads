...
def process_prism_data(
    ...
    if debug:
        for url_set in urls:
            proc_time_slice(
                url_set, indexer, fids, nc_dir, out_data, tmp_dir, overwrite
            )
...
def proc_time_slice(urls_, indexer_, fids_, nc_dir_, out_, temp, overwrite_):
        ...
        for var in PRISM_VARIABLES:
            ...
            for dt in time_coords:
                for da in var_dct[var]:
                    if da.time.values[0].astype("datetime64[D]") == np.datetime64(
                        dt, "D"
                    ):
                        if da.shape != (1, 621, 1405):
                            raise ValueError(
                                f"Unexpected shape in daily_data: {da.shape}"
                            )
                        ...
                        if first_da is None:
                            ...
                            if not np.array_equal(da.x.values, x_first):
                                print(
                                    f"Warning: Mismatched x coordinates for {var}, {dt}. Reassigning."
                                )
                                da = da.assign_coords(x=x_first)

                            if not np.array_equal(da.y.values, y_first):
                                print(
                                    f"Warning: Mismatched y coordinates for {var}, {dt}. Reassigning."
                                )
    ...
    ds = ds.sel(lat=indexer_.lat, lon=indexer_.lon, method="nearest", tolerance=4000.0)
        ...
        try:
            ...
            if not os.path.exists(_file) or overwrite_:
                ...
                if np.isnan(df_station[PRISM_VARIABLES].values.sum()) > 0:
                    ...
                    continue
                df_station["dt"] = [
                    dt.strftime("%Y%m%d%H") for dt in df_station["time"]
                ]
    ...
    gc.collect()
    print(
        f"wrote {ct} for {year_str}, skipped {skip}, {datetime.strftime(datetime.now(), '%Y%m%d %H:%M')}"
    )
