...
def get_daymet(start_date, end_date, down_dst=None):
    results = earthaccess.search_data(
        doi="10.3334/ORNLDAAC/2129", temporal=(start_date, end_date)
    )
    if down_dst:
        print("downloading netcdf")
...
def extract_daymet(
    ...
    if "LAT" in station_list.columns:
        station_list = station_list.rename(
            columns={"STAID": "fid", "LAT": "latitude", "LON": "longitude"}
        )
    ...
    station_list[["x", "y"]] = station_list.apply(
        projected_coords, axis=1, result_type="expand"
    )
    ...
    target_files = [
        [
            os.path.join(nc_dir, target_file_fmt.format("na", var_, year))
    ...
    if not all([os.path.exists(tp) for fl in target_files for tp in fl]):
        raise NotImplementedError("Missing files")

    if debug:
        for fileset, dts in zip(target_files[:1], years[:1]):
            proc_time_slice(
                fileset, indexer, dts, fids, out_data, overwrite, bounds_=None
            )
...
def proc_time_slice(
    fileset_, indexer_, date_string_, fids_, out_, overwrite_, bounds_=None
):
...
def projected_coords(row):
    ...
    target_crs = (
        "+proj=lcc +lat_0=42.5 +lon_0=-100 +lat_1=25 +lat_2=60 +x_0=0 "
        "+y_0=0 +ellps=WGS84"
    )
