...
def process_and_concat_csv(
    root, source, start_date, end_date, outdir, workers, missing_file=None, debug=False
):
    start = pd.to_datetime(start_date)
    end = pd.to_datetime(end_date)

    required_months = (
        pd.date_range(start=start, end=end, freq="MS").strftime("%Y%m").tolist()
    )
    required_years = (
        pd.date_range(start=start, end=end, freq="YE").strftime("%Y").tolist()
    )
    ...
    if debug:
        for sd in subdirs:
            if source == "nldas2":
                nldas2_parquet(
                    root, sd, required_months, expected_index, str_dtime, outdir
                )
            if source == "era5_land":
                era5_land_parquet(
                    root, sd, required_months, expected_index, str_dtime, outdir
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            if source == "nldas2":
                executor.map(
                    ...
                    [expected_index] * len(subdirs),
                    [str_dtime] * len(subdirs),
                    [outdir] * len(subdirs),
                )
...
def nldas2_parquet(
    ...
    if os.path.isdir(subdir_path):
        ...
        if os.path.exists(out_file) and csv_files_:
            ...
            print(
                f"{os.path.basename(out_file)} exists, removing {len(csv_files_)} csv files"
            )
            return
        ...
        if len(dtimes) < len(required_months_):
            ...
            if len(missing) > 0:
                print(
                    f"{subdir_} missing {len(missing)} months: {np.random.choice(missing, size=5, replace=False)}"
                )
                return
        ...
        shutil.rmtree(subdir_path)
        print(
            f"wrote {subdir_}, removed {len(rm_files)} .csv files,"
...
def era5_land_parquet(
    root_, subdir_, required_months_, expected_index_, strdt_, outdir_
):
    subdir_path = os.path.join(root_, subdir_)
    out_file = os.path.join(outdir_, f"{subdir_}.parquet.gzip")
    ...
    if os.path.isdir(subdir_path):
        ...
        dtimes = [
            "".join(f.split("_")[-2:]).replace(".parquet", "") for f in extract_files_
        ]
        ...
        if missing > 0:
            missing_idx = set(
                [f"{i.year}_{i.month}" for i in expected_index_ if i not in df.index]
            )
...
def general_parquet(root_, subdir_, required_years_, expected_index_, outdir_):
    ...
    if os.path.isdir(subdir_path):
        ...
        if os.path.exists(out_file) and csv_files_:
            shutil.rmtree(subdir_path)
            print(
                f"{os.path.basename(out_file)} exists, removing {len(csv_files)} csv files"
            )
            return
        ...
        if len(dtimes) < len(required_years_):
            ...
            if len(missing) > 0:
                if missing >= 5:
                    print(
                        f"{subdir_} missing {len(missing)} years: {np.random.choice(missing, size=5, replace=False)}"
                    )
                else:
