...
def get_nldas(start_date, end_date, down_dst=None):
    results = earthaccess.search_data(
        doi="10.5067/THUF4J1RLSYG", temporal=(start_date, end_date)
    )
    if down_dst:
...
def extract_nldas(
    ...
    if "END" in station_list.columns:
        ...
        station_list = station_list[
            station_list["end_dt"] > pd.to_datetime("1990-01-01")
        ]
    ...
    for year in range(1990, 2024):
        for month in range(1, 13):
            ...
            if nc_data:
                nc_files = [
                    f
                    for f in os.listdir(nc_data)
                    if date_string == f.split(".")[1][1:7]
                ]
                nc_files.sort()
                ...
                nc_files = get_nldas(
                    f"{year}-{m_str}-01", f"{year}-{m_str}-{month_end}"
                )
