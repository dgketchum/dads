...
class Thredds:
    ...
    def _warp(self):
        ...
        with rasopen(reproj_path, "w", **dst_profile) as dst:
            dst_array = empty(
                (src_array.shape[0], dst_height, dst_width), dtype=float32
            )
    ...
    def _mask(self):
        ...
        with rasopen(self.reprojection) as src:
            out_arr, out_trans = mask(
                src, self.clip_feature, crop=True, all_touched=True
            )
    ...
    def _resample(self):
        ...
        with rasopen(ras_obj, "r") as src:
            ...
            aff = src.transform
            new_affine = Affine(
                aff.a / res_coeff, aff.b, aff.c, aff.r, aff.e / res_coeff, aff.f
            )
...
class TopoWX(Thredds):
    ...
    def get_data_subset(
        self, grid_conform=False, var="tmax", out_file=None, temp_units_out="C"
    ):
    ...
    def _build_url(self, var):
        ...
        url = urlunparse(
            [
                self.scheme,
                self.service,
                "/thredds/dodsC/topowx?crs,lat[0:1:3249],lon[0:1:6999],{},time".format(
                    var
                ),
                "",
                "",
