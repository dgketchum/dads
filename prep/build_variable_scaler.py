...
from cube.model.value_limits import FEATURE_LIMITS, TARGET_LIMITS
...
def fit_and_save_scaler(
    file_paths, feature_names, scaler_path, num_workers=None, variable=None
):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as ex:
        ...
        for chunk in tqdm(
            ex.map(_read_parquet_features, args),
        ...
        for fname, lim in FEATURE_LIMITS.items():
            if fname in name_to_idx and lim is not None:
                ...
                if bad.any():
                    ...
                    feature_invalid_stats.append(
                        (fname, cnt, 100.0 * cnt / n_rows, lo, hi, mn, mx)
                    )
    ...
    if variable is not None:
        ...
        if lim is None:
            raise ValueError(
                f"TARGET_LIMITS not set for variable {variable}"
            )  # likely error if unset
        lo, hi = float(lim[0]), float(lim[1])
        x0 = full_dataset[:, 0]
        bad0 = (x0 < lo) | (x0 > hi)
    ...
    if feature_rows_masked.any():
        print(
            "[Scaler] rows masked by FEATURE_LIMITS:",
            int(feature_rows_masked.sum()),
            "/",
            int(n_rows),
        )
    ...
    print(
        "[Scaler] rows masked by TARGET_LIMITS:",
        int(target_rows_masked),
        "/",
        int(n_rows),
    )
    ...
    print(
        f"[Scaler] wrote new scaler with {len(feature_names)} features: {feature_names}"
    )
    return scaler


def build_variable_scaler(
    parquet_root,
    variable,
    scaler_dir=None,
    num_workers=None,
    ...
    files = [
        os.path.join(var_dir, f) for f in os.listdir(var_dir) if f.endswith(".parquet")
    ]
    ...
    assert (
        len(feature_names) >= 2
        and feature_names[0].endswith("_obs")
        and not feature_names[1].endswith("_obs")
    )
    ...
    _ = fit_and_save_scaler(
        files, feature_names, scaler_path, num_workers, variable=variable
    )
    return scaler_path
...
def load_variable_scaler(
    ...
    if not rebuild and os.path.exists(path):
        try:
            ...
            if "feature_names" in params and feature_names is not None:
                assert params["feature_names"] == feature_names, (
    ...
    assert (station_ids is not None) or (split_ids_path is not None), (
        "train_ids required for scaler rebuild"
    )
    path = build_variable_scaler(
        parquet_root, variable, station_ids=station_ids, split_ids_path=split_ids_path
    )
    with open(path, "r") as f:
        params = json.load(f)
    scaler = MinMaxScaler()
