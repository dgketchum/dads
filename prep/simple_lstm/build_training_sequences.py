...
KNOWN_REMAP = {'tmean_nl': 'mean_temp_nl'}
...
def join_training(stations, ts_dir, landsat_dir, dem_dir, out_dir, var='rsds',
    ...
    ts, ct, scaling, shape = None, 0, {}, None
    missing = {'sol_file': 0,
               'station_file': 0,
               'landsat_file': 0,
               'snotel': 0,
    ...
    for f, row in tqdm(stations.iterrows(), total=len(stations)):
        ...
        ts = ts.rename(columns=KNOWN_REMAP)
        ...
        # northern sites won't have gridmet
        try:
            ts = ts[[f'{var}_obs', f'{var}_gm', f'{var}_nl'] + feats]
        except KeyError:
            ts = ts[[f'{var}_obs', f'{var}_nl'] + feats]
        ...
        sol_file = os.path.join(dem_dir, f'{f}.csv')
