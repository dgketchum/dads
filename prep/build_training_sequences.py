...
def process_station(
    ...
    non_obs_cols = np.array(
        [
            c.split("_")[-1]
            for c in ts.columns
            if "obs" not in c and not c.endswith("_miss")
        ]
    )
        ...
        if obs_var in ts.columns and ts[obs_var].notna().any():
            ...
            if not var_df.empty:
                ...
                if not os.path.exists(out_file) or overwrite:
                    ...
                    print(
                        f"{fid}: {var_df.shape[0]} written to {out_file} {now}",
                        flush=True,
                    )
                else:
                    missing["exists"] += 1
...
def join_training(
    ...
    rows = [
        {
            "index": f,
            "latitude": float(row[kw["lat"]]),
            "longitude": float(row[kw["lon"]]),
            "orig_netid": str(row["orig_netid"]),
    ...
    if debug or workers <= 1:
        ...
        with multiprocessing.Pool(processes=workers) as pool:
            results = list(
                tqdm(pool.imap(process_station_wrapper, args), total=len(fids))
            )
...
def _encode_parquet_file(p):
    ...
    ):  # likely error if index isn't datetime
...
def add_doy_encoding(parquet_root, num_workers=32):
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        for _ in tqdm(
            pool.imap_unordered(_encode_parquet_file, files, chunksize=64),
            total=len(files),
        ):
            pass
