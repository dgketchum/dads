...
def _join_one_worker(args):
    f, row, sta_dir, gridded_met_dir, dst_dir, source, overwrite, hourly, clip_to_obs = args
    ...
    try:
        if hourly:
            gridded_met_file = os.path.join(hourly_, '{}.parquet.gzip'.format(f))  # likely error: hourly_ not in scope
        ...
        empty_local, eidx_local = add_empty_entry(empty_local, eidx_local, f, row, 'does not exist', gridded_met_file)
        print('gridded_file {} does not exist'.format(os.path.basename(gridded_met_file)))
        nld_cols = ['{}_e5l_hr'.format(c) for c in OBS_TARGETS] if hourly else ['{}_e5l'.format(c) for c in OBS_TARGETS]
        ndf = None

    except pyarrow.lib.ArrowInvalid:
        empty_local, eidx_local = add_empty_entry(empty_local, eidx_local, f, row, 'is empty', gridded_met_file)
    ...
    if sdf.empty:
        empty_local, eidx_local = add_empty_entry(empty_local, eidx_local, f, row, 'col all nan', sta_file)
...
def join_daily_timeseries(stations, sta_dir, gridded_met_dir, dst_dir, source,
                          ...
                          hourly=False, clip_to_obs=True, num_workers=12):
    """"""
    ...
    iterable = [(f, row, sta_dir, gridded_met_dir, dst_dir, source, overwrite, hourly, clip_to_obs)
    ...
    if int(num_workers) == 1:
        ...
        with ProcessPoolExecutor(max_workers=int(num_workers)) as ex:
            for i, (inc, miss_df) in enumerate(ex.map(_join_one_worker, iterable), start=1):
                ct += inc
                if not miss_df.empty:
                    empty = pd.concat([empty, miss_df], ignore_index=True)
                if i % 50 == 0:
...
if __name__ == '__main__':
    ...
    join_daily_timeseries(sites, obs, daily, joined, source=src_, overwrite=overwrite,
                          ...
                          clip_to_obs=clip_to_obs_, num_workers=12)
