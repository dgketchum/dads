...
from cube.model.dataset import DadsDataset
...
def train_model(
    ...
    _req = [
        "train_edge_index.json",
        "train_edge_attr.json",
        "val_edge_index.json",
        "val_edge_attr.json",
    ]
    for _f in _req:
    ...
    all_files = {
        os.path.splitext(f)[0]: os.path.join(parquet_dir, f)
    ...
    train_dataset = DadsDataset(
        ...
        train_edges,
        train_attr,
        scaler=scaler_json_path,
        sample=sample[0],
        lstm_workers=n_workers,
        normalize_keys=train_stations,
        windows_per_station=(
            64 if debug and n_samples is not None else windows_per_station
        ),
    ...
    val_dataset = DadsDataset(
        ...
        normalize_keys=train_stations,
        windows_per_station=(
            16 if debug and n_samples is not None else windows_per_station
        ),
        neighbor_file_map=all_files,
    )
    ...
    def print_dads_training_overview(tr_ds, va_ds, n_nodes):
        ...
        print(
            f"- Train stations: {len(tr_ds.file_paths)}  Val stations: {len(va_ds.file_paths)}"
        )
        ...
        print(
            f"- Edge attr dim: {tr_ds.edge_dim}  (bearing={'on' if has_bear else 'off'}, distance={'on' if has_dist else 'off'})"
        )
        ...
        if has_dist and isinstance(tr_ds._distance_map, dict):
            try:
                ...
                if vals:
                    ...
                    print(
                        f"- Distance (km) summary over sample edges: min={arr.min():.1f}, median={np.median(arr):.1f}, max={arr.max():.1f}"
                    )
            except Exception:
                pass
    ...
    edge_dim = int(
        getattr(
            train_dataset,
            "edge_dim",
            next(iter(train_dataset.edge_attr.values())).shape[-1],
        )
    )
    ...
    model = DadsMetGNN(
        ...
        log_csv=logging_csv,
        use_target_exog_branch=True,
        emb_dim=emb_dim,
        exog_dim=exog_dim,
        scaler=scaler_obj,
        column_indices=column_indices,
        tcn_in_channels=tcn_in_channels,
...
if __name__ == "__main__":
    ...
    train_model(
        ...
        device=device_,
        n_samples=(N_SAMPLES if DEBUG else None),
        debug=DEBUG,
