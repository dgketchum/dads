...
def get_station_metadata(
    ...
    if time_constraint_yearmonths:
        if isinstance(time_constraint_yearmonths, (list, tuple)) and len(time_constraint_yearmonths) == 2:
            ...
            if csv_file.endswith(".csv"):
                ...
                if len(parts) > 1:
                    ...
                    if start_ym_str_constraint and end_ym_str_constraint:
                        if not (start_ym_str_constraint <= yearmonth <= end_ym_str_constraint):
                            continue
    ...
    while any(monthly_targets_remaining.values()):
        active_monthly_targets = {ym: stations for ym, stations in monthly_targets_remaining.items() if stations}
        if not active_monthly_targets:
            break

        yearmonth_to_process = max(active_monthly_targets, key=lambda k: len(active_monthly_targets[k]))
        current_targets_for_this_ym_iteration = set(monthly_targets_remaining[yearmonth_to_process])
        ...
        for f_path in all_gz_files_in_datadir:
            if yearmonth_to_process in os.path.basename(f_path):
                ...
                yearmo_nc_files_for_processing.append({'path': f_path, 'size': file_size})

        sorted_yearmo_nc_files = [
            item['path'] for item in sorted(yearmo_nc_files_for_processing, key=lambda x: x['size'], reverse=True)
        ]
        ...
        for enum, filename in enumerate(sorted_yearmo_nc_files, start=1):
            ...
            for station_id_in_file in df.index:
                if station_id_in_file in current_targets_for_this_ym_iteration:
                    ...
                    if station_id_in_file in station_dct:
                        monthly_targets_remaining[yearmonth_to_process].discard(station_id_in_file)
                        continue

                    row = df.loc[station_id_in_file]
                    stype_val = row['stationType']
                    stype_str = (stype_val.decode('utf-8', errors='replace')
            ...
            if file_updated_tracker:
                print(f'{os.path.basename(filename)}: {enum} of {len(sorted_yearmo_nc_files)}, '
                      f'{len(station_dct)} stations', flush=True)
                with open(output_station_tracker_path, 'w') as fp:
                    json.dump(station_dct, fp, indent=4)

            if stations_processed_in_current_nc_loop.issuperset(current_targets_for_this_ym_iteration):
                break
