...
def process_single_station(station_id, ndbc_src, ndbc_dst, stations_meta, overwrite=False, min_obs=4, debug=False):
    ...
    try:
        ...
        print(f"NDBC {station_id}: {in_fp} rows={obs_total_} days={days_} "
    ...
    num_cols = [
        'air_temp', 'dewpoint', 'pressure', 'water_temp', 'wave_height',
        'dominant_wave_period', 'average_wave_period', 'mean_wave_dir',
        'visibility', 'tide'
    ]
    for c in num_cols:
        if c in df.columns:
    ...
    if daily_df.empty:
        ...
        try:
            ...
            print(f"NDBC {station_id}: empty daily after filtering. df rows={len(df)} cols={len(cols_)} "
...
def process_ndbc_daily(ndbc_src, ndbc_dst, stations_meta_csv, station_ids=None, overwrite=False,
                       min_obs=4, num_workers=12, debug=False):
    ...
    if debug or int(num_workers) == 1:
        for sid in station_ids:
            process_single_station(sid, ndbc_src, ndbc_dst, meta, overwrite=overwrite, min_obs=min_obs, debug=debug)
    else:
        args_iter = [(sid, ndbc_src, ndbc_dst, meta, overwrite, min_obs, debug) for sid in station_ids]
...
def write_ndbc_hourly(ndbc_src, out_root, stations_meta_csv, station_ids=None):
    ...
    for sid in station_ids:
        ...
        keep_extra = [
            'pressure', 'water_temp', 'wave_height', 'dominant_wave_period',
            'average_wave_period', 'mean_wave_dir', 'visibility', 'tide'
        ]
        ...
        final_cols = ['datetime', 'stationId', 'elevation', 'latitude', 'longitude', 'code1PST', 'code2PST']
...
if __name__ == '__main__':
    ...
    process_ndbc_daily(ndbc_src_dir, ndbc_daily_dir, ndbc_meta_csv, overwrite=False, min_obs=4)
