...
NDBC_VARS = [
    'tmax', 'tmin', 'tmean', 'wind', 'wind_dir', 'u', 'v',
    'dewpoint', 'pressure', 'water_temp', 'wave_height',
...
def assess_downloaded_ndbc(records_dir, out_csv=None, joined_dir=None, training_dir=None,
                           landsat_dir=None, cdr_dir=None, solrad_dir=None, terrain_dir=None,
    ...
    if num_workers is None or num_workers <= 1:
            ...
            if row:
                ...
                if training_dir:
                    ...
                    try:
                        ...
                        for s in subs:
                            _promote_lowercase_file(os.path.join(training_dir, s), stn, 'parquet')
        ...
        with ProcessPoolExecutor(max_workers=int(num_workers)) as ex:
            for row in tqdm(ex.map(_assess_ndbc_file, files), total=len(files), desc='Assessing NDBC'):
                if row:
                    stn = row['station']
                    if joined_dir:
                        _promote_lowercase_file(joined_dir, stn, 'parquet')
...
if __name__ == '__main__':
    ...
    assess_downloaded_ndbc(ndbc_daily,
                           out_csv_,
                           joined,
                           training,
                           landsat_,
