...
import process.obs.qaqc.plot as plotting_functions
from process.obs.qaqc.utils import get_int_input, get_float_input, FEATURES_DICT
import warnings
...
def rs_period_ratio_corr(start, end, rs, rso, sample_size_per_period, period):
    """
    ...
    RULE 1:
        ...
        the value of that correction factor if sensor drift/miscalibration has occurred. While there may be variation
        between individual solar radiation observations due to cloudiness, over the period each cloud-free day
        should have approximately the same ratio with its corresponding clear-sky solar radiation value. This logic
        also holds true when parsing a period that needs to drift adjustment but may have bad observations.

    RULE 2:
        Is the average Rs value (calculated from the underlying Rs values from the Rs/Rso ratios included in the
        correction factor) exceed the average Rso value by at least 75 w/m**2? The main purpose of this rule is to
        warn the user when RULE 1 has been satisfied but this rule has not. This would occur when the observational
        data has a high concentration of bad values. The recommended course of action when this rule is violated
        is to remove the data manually.

    If RULE 1 violated, is this whole process is repeated with the next largest ratio until the correction factor
    doesn't significantly change. Values determined to be bad are set to a marker value and then
    later set to be equal to Rso * 1.05.

    Example:
        sorted_ratio_list = [3, 1.5, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.7, 0.7...]

        We have the original correction factor of the 1st-6th largest rs/rso ratios, and we compute it again
        by dropping the largest ratio and including the next largest. Ex: 1st-6th largest would become 2nd-7th largest.
        We check to see if this shifted in included values causes a larger than 2% change in the correction factor,

        Correction factor does significantly change when computed between values 1st-6th and 2nd-7th, with the same
        behavior between 2nd-7th and 3rd-8th. The correction factor doesn't change significantly
        when computed between values 3rd-8th and 4th-9th.

        The correction factor that will be applied to the data will be based on values 3rd-8th, and the values for
    ...
    print('Periodic ratio-based Rs corrections were applied,'
                     ' period length was %s, and correction sample size was %s. \n'
                     % (period, sample_size_per_period))
    print('%s data points were removed as part of the suspect value evaluation process. \n'
                     % bad_vals_counter)
    print('%s Rs data points in %s different periods were removed due to insufficient data present in'
                     ' either Rs or Rso to compute a correction factor. \n'
                     % (insufficient_data_counter, insufficient_period_counter))
    print('%s data points were removed due to their correction factor exceeding a '
                     '50 percent relative increase or decrease. \n' % correction_cutoff_counter)
