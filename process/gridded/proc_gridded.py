    ...
    for i, (fid, row) in enumerate(station_list.iterrows(), start=1):
        ...
        if targets['nldas2']:
            ...
            if not os.path.exists(in_file_):
                print(f'NLDAS at {os.path.basename(in_file_)} does not exist, skipping, {lat:.1f} {lon:.1f}')

            else:
                if hourly:
                    out_file_ = os.path.join(gridded_dir, 'processed_parquet', 'nldas2_hourly',
                                             '{}.parquet'.format(fid))
                else:
                ...
                if not os.path.exists(out_file_) or overwrite:
                    proc_nldas(in_file_=in_file_, lat=lat, elev=elv, out_file_=out_file_,
        ...
        if targets['gridmet']:
            ...
            if not os.path.exists(in_file_):
                print(f'GridMET at {os.path.basename(in_file_)} does not exist, skipping, {lat:.1f} {lon:.1f}')
                ...
                if not os.path.exists(out_file_gm) or overwrite:
                    proc_gridmet(in_file_=in_file_, lat=lat, elev=elv, out_file_=out_file_gm, parallel=pd_parallel)
                    print('gridmet', fid)
                else:
                    pass
        ...
        if targets['prism']:
            ...
            if not os.path.exists(in_file_):
                print(f'PRISM at {os.path.basename(in_file_)} does not exist, skipping, {lat:.1f} {lon:.1f}')

            else:
                out_file_prism = os.path.join(gridded_dir, 'processed_parquet', 'prism', '{}.parquet'.format(fid))
        ...
        if targets['daymet']:
            ...
            if not os.path.exists(in_file_):
                print(f'DAYMET at {os.path.basename(in_file_)} does not exist, skipping, {lat:.1f} {lon:.1f}')

            else:
                out_file_dm = os.path.join(gridded_dir, 'processed_parquet', 'daymet', '{}.parquet'.format(fid))
...
def proc_daymet(in_file_, elev, out_file_):
    try:
        ...
        df['ea'] = calcs._actual_vapor_pressure(pair=calcs._air_pressure(elev), q=df['q'])
