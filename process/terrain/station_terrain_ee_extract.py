...
def write_station_terrain(tile_dirs, station_out, shuffle=False, overwrite=False, index_col='fid',
                          coords=['latitude', 'longitude'], num_workers=12):
    ...
    for tdir in tile_dirs:
        if os.path.isdir(tdir):
            csv_files.extend([os.path.join(tdir, f) for f in os.listdir(tdir) if f.endswith('.csv')])
    ...
    if num_workers is None or int(num_workers) <= 1:
        ...
        with ProcessPoolExecutor(max_workers=int(num_workers)) as ex:
            for _ in tqdm(ex.map(_process_csv_file, jobs), total=len(jobs), desc='Writing terrain'):
                pass
