...
def join_stations(
    snotel, mesonet, agrimet, out_file, fill_elevation=False, bounds=None
):
    """"""
    ...
    snotel_data = snotel_data.rename(
        columns={
            "ID": "fid",
    ...
    snotel_data = snotel_data[
        ["fid", "name", "elevation", "latitude", "longitude", "source", "orig_netid"]
    ]
        ...
        for e, (i, r) in enumerate(d.items(), start=1):
            try:
                if r["elevation"] > 4200.0 or r["elevation"] < 0.0:
                    d[i]["elevation"] = elevation_from_coordinate(
...
if __name__ == "__main__":
    ...
    agrim_list = os.path.join(
        d, "dads", "met", "stations", "openet_gridwxcomp_input.csv"
    )

    stations = os.path.join(d, "dads", "met", "stations", "dads_stations_10FEB2025.shp")
    join_stations(
        sno_list, meso_list, agrim_list, stations, bounds=None, fill_elevation=False
    )
