...
def download_ghcn(station_id, file_dst):
    url = "https://www.ncei.noaa.gov/pub/data/ghcn/daily/by_station/{}.csv.gz".format(
        station_id
    )
    ...
    df = df.pivot_table(
        index="DATE", columns=["PARAM"], values="VALUE", aggfunc="first"
    ).reindex()
...
def get_station_data(
    ...
    stations = stations[
        (stations["latitude"] < bounds[3]) & (stations["latitude"] >= bounds[1])
    ]
    stations = stations[
        (stations["longitude"] < bounds[2]) & (stations["longitude"] >= bounds[0])
    ]
...
if __name__ == "__main__":
    ...
    get_station_data(
        inventroy_, rec_dir, overwrite=False, tracker=tracking, shuffle=True
    )
