...
def download_county_air_quality_data(
    key_file, state_fips, county_fips, start_date, end_date, data_dst, overwrite=False
):
    """"""
    st_fips = {v: k for k, v in state_fips_code().items()}

    URL = (
        "https://aqs.epa.gov/data/api/dailyData/byCounty?email={a}&"
        ...
        for yr in years:
            ...
            try:
                ...
                for site_no in sites:
                    ...
                    c = c.groupby("date").agg(
                        {obsname: "first", "latitude": "first", "longitude": "first"}
                    )
                    c.to_csv(file_)
...
def join_aq_data(data_src, data_dst, out_csv):
    ...
    blank = pd.DataFrame(
        columns=[v for k, v in AQS_PARAMETERS.items()], index=pd.DatetimeIndex(dt)
    )
    ...
    for site in tqdm(sites, total=len(sites)):
        ...
        filepaths = [
            ...
            if site in f and f.endswith(".csv")
        ]
