...
def process_daily_data(raw_df, rsun_data, lat_, elev_, zw_=2.0, qaqc=False):
    ...
    daily_prcp = (
        hourly_df.groupby("date")
        .apply(calculate_daily_precipitation, include_groups=False)
        .to_frame("prcp")
    )
    ...
    if qaqc:
        ...
        if daily_df["rsds"].notna().any():
            daily_df["rolling_rsds_max"] = (
                daily_df["rsds"].rolling(15, min_periods=1).max()
            )
            daily_df["rolling_rsds_max"] = daily_df["rolling_rsds_max"].bfill().ffill()
            daily_df.loc[
                daily_df["rolling_rsds_max"].isna()
                | (daily_df["rolling_rsds_max"] == 0),
                "rolling_rsds_max",
            ] = np.nan
...
def process_single_station(
    fid, madis_src, madis_dst, rsun_tables, overwrite, qaqc, plot, alt_src
):
    """
    ...
    try:
        ...
        print(
            f'{fid}: Failed to extract metadata. Ensure "longitude", "latitude", and "elevation" exist. Error: {e}'
        )
        return
    ...
    if df is None or df.empty:
        print(
            f"{fid}: No processable records found after daily aggregation", flush=True
        )
        return
...
def plot_daily_data(pdf, station_id, year, out_fig):
    ...
    fig.suptitle(
        f"Daily Meteorological Data for Station {station_id} - {year}", fontsize=14
    )
...
if __name__ == "__main__":
    ...
    madis_hourly_public = os.path.join(
        d, "climate", "madis", "LDAD_public", "mesonet", "csv"
    )
