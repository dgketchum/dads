...
def read_gwx(stations, target_sites, gwx_src, gwx_dst, overwrite=False):
    ...
    for i, row in network_sites.iterrows():
        ...
        _, lat, elv, zw = row[kw["lon"]], row[kw["lat"]], row[kw["elev"]], 2.0
        ...
        asce_params = df.parallel_apply(
            calc_asce_params, lat=lat, elev=elv, zw=zw, axis=1
        )
        # asce_params = df.apply(calc_asce_params, lat=lat, elev=elv, zw=zw, axis=1)

        try:
            df[["mean_temp", "vpd", "rn", "u2", "eto"]] = pd.DataFrame(
...
if __name__ == "__main__":
    ...
    dads_stations = os.path.join(
        d, "dads", "met", "stations", "dads_stations_elev_mgrs.csv"
    )
