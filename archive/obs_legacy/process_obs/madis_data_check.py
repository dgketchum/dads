...
def assess_downloaded_madis(
    ...
    if stations_csv and os.path.exists(stations_csv):
        try:
            ...
            if (
                start_col
                and end_col
                and start_col in st_df.columns
                and end_col in st_df.columns
            ):
    ...
    if num_workers is None or num_workers <= 1:
        ...
        with ProcessPoolExecutor(max_workers=int(num_workers)) as ex:
            for row in tqdm(
                ex.map(_assess_madis_file, files),
                total=len(files),
                desc="Assessing MADIS",
            ):
                if row:
                    stn = row["fid"]
                    row["in_joined"] = int(
