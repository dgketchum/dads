...
def open_nc(f):
    ...
    try:
        ...
        try:
            ...
            print(
                f"Error writing to temporary .nc file or reading with netCDF4 for {f}: {e2}"
            )
            return None
        finally:
...
def read_madis_hourly(
    data_directory,
    year_mo_str,
    output_directory,
    bounds=(-125.0, 24.0, -66.0, 53.0),
    select=None,
):
    """"""
    ...
    for filename in file_list:
        ...
        try:
            pstdct = {
                k: [int(i) for i in ds[k].values if np.isfinite(i)]
                for k in PST
                if "code" in k
            }
        except KeyError:
        ...
        [
            pstdct.update(
                {
                    k: [i.strip() for i in ds[k].values.astype(str) if i != ""]
                    for k in PST
                    if "code" not in k
                }
            )
        ...
        for var_name, var_type in params_to_extract.items():
            if var_name in ds:
                if var_name == "observationTime":
                    data_vars[var_name] = pd.to_datetime(
                        ds[var_name].values, errors="coerce"
                    )
                elif var_type is str:
                    data_vars[var_name] = [
                        val.decode().strip() for val in ds[var_name].values
                    ]
                else:
                    data_vars[var_name] = ds[var_name].values.astype(var_type)
            else:
                if "recNum" in ds.dims:
                    default_val = "" if var_type is str else np.nan
        ...
        df = pd.merge(
            df, pst_map_df, left_on="dataProvider", right_on="namePST", how="left"
        )
    ...
    dtstr = current_datetime.strftime("%Y-%m-%d %H:%M")
    print(
        f"{year_mo_str}: read and concatenated {len(file_list)} files, writing stations {dtstr}",
        flush=True,
    )
    ...
    for station_id, station_df in master_df.groupby("stationId"):
        try:
            ...
            final_cols = ["datetime"] + [
                c for c in DESIRED_PARAMS.keys() if c in station_df.columns
            ]
    ...
    dtstr = current_datetime.strftime("%Y-%m-%d %H:%M")
    print(
        f"{year_mo_str}: {len(file_list)} files took {end - start:0.4f} seconds {dtstr}",
        flush=True,
    )
...
if __name__ == "__main__":
    ...
    if os.path.isfile(log):
        with open(log, "r") as fp:
            lines = fp.readlines()
            lines = [ln for ln in lines if "files took"]

        complete_yrmos = [ln[:6] for ln in lines]
    else:
        complete_yrmos = None
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        future_to_args = {
            executor.submit(process_time_chunk, arg): arg for arg in args_
        }
