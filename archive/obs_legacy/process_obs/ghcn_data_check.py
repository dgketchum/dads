...
def assess_downloaded_data(
    ...
    if num_workers is None or num_workers <= 1:
        ...
        with ProcessPoolExecutor(max_workers=int(num_workers)) as ex:
            for row in tqdm(
                ex.map(_assess_station_file, files),
                total=len(files),
                desc="Assessing stations",
            ):
                if row:
                    stn = row["station"]
                    se = start_end_map.get(stn, ("NULL", "NULL"))
    ...
    in_joined = (
        summary_df.get("in_joined", pd.Series(0, index=summary_df.index))
        .fillna(0)
        .astype(int)
    )
    in_training = (
        summary_df.get("in_training", pd.Series(0, index=summary_df.index))
        .fillna(0)
        .astype(int)
    )

    remain = pd.Series(
        True, index=summary_df.index
    )  # include pre-1987 in diagnostics post pre-RS acceptance
    ...
    def mcount(col):
        s = (
            summary_df.get(col, pd.Series(0, index=summary_df.index))
            .fillna(0)
            .astype(int)
        )
    ...
    if var_cols:
        ...
        for c in var_cols:
            tot = (
                pd.to_numeric(summary_df[c], errors="coerce")
                .fillna(0)
                .astype(int)
                .sum()
            )
    ...
    if out_shp:
        ...
        try:
            ...
            inv_df = pd.DataFrame(
                [row.split() for row in inv_lines],
                columns=[
                    "station",
                    "latitude",
                    "longitude",
            ...
            inv_df = inv_df.drop_duplicates(subset=["station"])[
