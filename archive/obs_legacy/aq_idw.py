...
def csv_to_netcdf(in_dir, metadata, output_file):
    ...
    aqm.index = aqm.apply(
        lambda r: (
            f"{str(r['st_fips']).rjust(2, '0')}"
            f"{str(r['co_fips']).rjust(3, '0')}"
            f"{str(r['site_no']).rjust(4, '0')}"
        ),
        axis=1,
    )
    ...
    fid_to_coords = {
        fid: (lat, lon)
        for fid, lat, lon in zip(aqm.index, aqm["latitude"], aqm["longitude"])
    }
...
def calculate_raster(
    data, parameter, resolution=0.05, lat=(42.0, 49.0), lon=(-115, -103), tif=None
):
    """"""
    ...
    obs_coords = np.column_stack(
        (param_data["longitude"].values, param_data["latitude"].values)
    )
    obs_values = param_data.values

    tree = cKDTree(obs_coords)
    distances, indices = tree.query(
        np.column_stack((grid_lon.ravel(), grid_lat.ravel())), k=len(obs_coords)
    )
...
def interpolate_at_points(
    ...
    idw_raster, latitudes, longitudes = calculate_raster(
        data, parameter, resolution, lat, lon, tif=out_raster
    )
...
def interpolate_station_aq(nc_file, output, stations, raster_dst=None):
    ...
    stations = stations[
        (stations["latitude"] < north) & (stations["latitude"] >= south)
    ]
    stations = stations[
        (stations["longitude"] < east) & (stations["longitude"] >= west)
    ]
    ...
    for year in range(2000, 2025):
        tasks.append(
            process_year(
                year, mds, points, params, south, north, west, east, output, raster_dst
            )
        )
...
@dask.delayed
def process_year(
    year, mds, points, params, south, north, west, east, ouput_dir, raster_dst
):
    ...
    for d, day in enumerate(ds.date.values):
        ...
        for i, parameter in enumerate(params):
            ...
            try:
                interp_vals = interpolate_at_points(
                    daily_data,
                    parameter,
                    points.values,
                    resolution=0.05,
