...
def _process_tile(args):
    ...
    if not os.path.exists(test_file):
        ...
        return tile
    complete = check_exists(
        rs_file=test_file, out_directory=out_dir, index_col=index_col
    )
...
def process_landsat(
    ...
    _ts, _ct, scaling, _first, _shape = None, 0, {}, True, None
    ...
    if num_workers == 1:
        ...
        total = len(tiles)
        args = [
            (tile, i, total, rs_dir, out_dir, glob, overwrite, extrapolate, index_col)
            for i, tile in enumerate(tiles, start=1)
        ]
...
def build_landsat_tables(rs_directory, _tile, glob, index_col, extrapolate=False):
    ...
    rs_files = [
        (y, os.path.join(rs_directory, f"{glob}_500_{y}_{_tile}.csv"))
    ...
    for y, f in rs_files:
        ...
        for index, row in mdf.iterrows():
            ...
            ls_vars = [
                c if c.split("_")[-1] in periods.keys() else None for c in df.index
            ]
            ...
            if not all(exist) and extrapolate:
                ...
                def repeat_data_for_year(year):
                    new_df = df.copy()
                    new_df.index = new_df.index.map(
                        lambda d: _safe_replace_year(d, year)
                    )
                    return new_df
...
if __name__ == "__main__":
    ...
    if "madis" in glob_:
        raise ValueError(
            "MADIS data needs to be appended to exising files, functionality that does not exist"
        )
    process_landsat(
        fields,
        rs,
