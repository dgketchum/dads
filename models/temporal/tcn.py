...
class TemporalBlock(nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=1, dropout=0.1, groups=8):
        super().__init__()
        c_in, c_out = channels
        self.conv1 = _CausalConv1d(c_in, c_out, kernel_size=kernel_size, dilation=dilation)
        self.norm1 = nn.GroupNorm(num_groups=min(groups, c_out), num_channels=c_out)
        self.conv2 = _CausalConv1d(c_out, c_out, kernel_size=kernel_size, dilation=dilation)
...
class TemporalConvEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        c_prev = in_channels
        for d in dilations:
            layers.append(TemporalBlock((c_prev, channels), kernel_size=kernel_size, dilation=d, dropout=dropout))
            c_prev = channels
