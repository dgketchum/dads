...
)  # likely error if CUDA not available
...
def train_model(
    ...
    debug_max_train_files=None,
    debug_max_val_files=None,
    use_compile=False,
    max_epochs=None,
    autotune_warmup=False,
    min_target_frac=0.8,
    require_rs_present=True,
    ...
    def print_autoencoder_training_overview(
        parquet_dir, cols_cfg, actual_cols, selected_idx
    ):
        ...
        try:
            ...
            df = pd.read_parquet(
                sample_file, columns=[c for c in subset if c in actual_cols]
            )
            desc = df.describe().T
            ...
            for col in df.columns.tolist():
                ...
                print(
                    f"  {col}: count={int(row['count'])}, non_null={nnv:.2f}, min={row['min']:.3f}, max={row['max']:.3f}"
                )
        except Exception:
            pass
    ...
    for filename in os.listdir(parquet):
        ...
        (
            file_map["train"]
            if len(file_map["train"]) <= len(file_map["val"])
            else file_map["val"]
        ).append(os.path.join(parquet, filename))

    sample = (
        file_map["train"][0]
        if file_map["train"]
        else (file_map["val"][0] if file_map["val"] else None)
    )
    if sample is None:
    ...
    if os.path.exists(train_ids_path_) and os.path.exists(val_ids_path_):
        try:
            ...
            all_paths_ = [
                os.path.join(parquet, f)
                for f in os.listdir(parquet)
                if f.endswith(".parquet")
            ]
            id_to_path_ = {
                os.path.splitext(os.path.basename(p))[0]: p for p in all_paths_
            }
            file_map = {
    ...
    if (
        (debug_max_files is not None)
        or (debug_max_train_files is not None)
        or (debug_max_val_files is not None)
    ):
        ...
        print(
            f"Debug mode: limiting files -> train: {len(file_map['train'])}, val: {len(file_map['val'])}"
        )
    ...
    train_dataset = WeatherIterableDataset(
        ...
        max_files=(
            int(debug_max_train_files) if debug_max_train_files is not None else None
        ),
        split_name="train",
        shuffle_files=True,
    ...
    def _collate(batch):
        ...
        p_list = [
            torch.full_like(xi, torch.nan) if pi is None else pi
            for xi, pi in zip(x_list, p_list)
        ]
        n_list = [
            torch.full_like(xi, torch.nan) if ni is None else ni
            for xi, ni in zip(x_list, n_list)
        ]
    ...
    val_dataset = WeatherIterableDataset(
        ...
        triplet_sampling=False,
        max_samples=(
            max(1, int(debug_n_samples // 4))
            if isinstance(debug_n_samples, int) and debug_n_samples > 0
            else None
        ),
        max_files=(
            int(debug_max_val_files) if debug_max_val_files is not None else None
        ),
        split_name="val",
        shuffle_files=False,
        min_target_frac=float(min_target_frac),
    ...
    if use_compile and not _short_debug:
        ...
        print(
            "Debug mode: skipping torch.compile to avoid potential cudagraph/dynamo issues."
        )
    ...
    if warmup_toggle_epochs is not None and int(warmup_toggle_epochs) > 0:
        ...
        class ZeroTargetToggle(_pl.Callback):
            ...
            def on_train_epoch_end(self, trainer, pl_module):
                ...
                if (
                    trainer.current_epoch + 1 >= self.toggle_epoch
                    and getattr(pl_module, "zero_target_in_encoder", None) is False
                ):
        ...
        trainer_kwargs["callbacks"] = trainer_kwargs["callbacks"] + [
            ZeroTargetToggle(warmup_toggle_epochs)
        ]

    # If short debug AND no explicit file limits provided, cap batches
    if _short_debug and (
        debug_max_files is None
        and debug_max_train_files is None
        and debug_max_val_files is None
    ):
...
def debug_sample(
    ...
    **kwargs,
    ...
    return train_model(
        ...
        debug_max_train_files=(
            int(n_train_files) if n_train_files is not None else None
        ),
