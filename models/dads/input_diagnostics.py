...
from prep.columns_desc import GEO_FEATURES, CDR_FEATURES, RS_MISS_FEATURES
...
def diagnose_inputs(training_root, variable, num_workers=12):
    ...
    seq_cols = [c for c in cols if (c == target or c in (['rsun'] + list(CDR_FEATURES)))]
    exog_cols = [c for c in GEO_FEATURES if c in cols]
    ...
    if num_workers and num_workers > 1:
        ...
        for fp in files:
            ...
            for c in report_cols:
                ...
                data[c].append(v)
    data = {c: (np.concatenate(v) if v else np.zeros((0,), dtype=np.float32)) for c, v in data.items()}
    ...
    if d_vals:
        ...
        print('\n[Edges] distance_km: min={:.3f} median={:.3f} max={:.3f}'.format(np.nanmin(d), np.nanmedian(d), np.nanmax(d)))
    if b_vals:
        b = np.concatenate(b_vals)
        print('[Edges] bearing_deg: min={:.3f} median={:.3f} max={:.3f}'.format(np.nanmin(b), np.nanmedian(b), np.nanmax(b)))
