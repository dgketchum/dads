...
CATALOG_URL = (
    "https://www.ncei.noaa.gov/thredds/catalog/cdr/surface-reflectance/{}/catalog.html"
)


def get_catalog(out_meta, start_year=1990, end_year=2025):
    catalog = {}
...
def download_chunk(chunk):
    ...
    if not all([os.path.exists(f) for f in targets]):
        ...
        print(
            f"Download {os.path.basename(targets[0])} : {os.path.basename(targets[-1])}"
        )
    else:
        print("chunks exist")


def download(data_dir, meta, num_proc, chunk_len=5, start_year=1990, end_year=2025):
    ...
    for year in range(start_year, end_year):
        ...
        all_targets = [
            os.path.join(data_dir, v[0]) for k, v in catalog[str(year)].items()
        ]

        urls_chunked = [
            all_urls[i : i + chunk_len] for i in range(0, len(all_urls), chunk_len)
        ]
        targets_chunked = [
            all_targets[i : i + chunk_len]
            for i in range(0, len(all_targets), chunk_len)
        ]
