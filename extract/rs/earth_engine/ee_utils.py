...
def landsat_composites(year, start, end, append_name, scale=False):
    ...
    thermal = ee.Image(
        ...
        .map(lambda x: x.select(["B10"], ["B10_{}".format(append_name)]))
        .mean()
    )
...
def is_authorized(project=None):
    if project is None:
        project = os.environ.get("EEPROJECT")
    if not project:
        raise RuntimeError(
            "Earth Engine project not set. Export EEPROJECT=<your-project-id> "
            "(e.g. EEPROJECT=ee-dgketchum)"
        )
    try:
