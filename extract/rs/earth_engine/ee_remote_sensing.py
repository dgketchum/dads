...
def request_updates_from_shapefile(
    stations_shp,
    index_col,
    station_out_dir,
    file_prefix,
    ...
    if tiles is None:
        tiles = [
            m for m in stations["MGRS_TILE"].unique().tolist() if isinstance(m, str)
        ]
    ...
    request_band_extract(
        ...
        buffer,
        run_tiles,
        check_dir=update_root,
        export_tif=False,
        dry_run=dry_run,
    )


def request_updates_from_missing_list(
    sites_path, index_col, missing_csv, file_prefix, points_layer, buffer, update_root
):
    ...
    if m.empty:
        return
    groups = (
        m.groupby("MGRS_TILE")["year"].apply(lambda s: sorted(list(set(s)))).to_dict()
    )
...
if __name__ == "__main__":
    ...
    if station_set == "madis":
        ...
        sites_shp = os.path.join(
            d, "climate", "ghcn", "stations", f"{stations_glob}.shp"
        )
        ...
        sites_shp = os.path.join(
            d, "climate", "ndbc", "ndbc_meta", "ndbc_stations_simple.shp"
        )
    ...
    request_band_extract(
        stations_glob,
        points_layer=ee_points,
        years=years_,
        buffer=500.0,
        tiles=mgrs_tiles,
        check_dir=updates_root,
        export_tif=False,
        dry_run=False,
    )
