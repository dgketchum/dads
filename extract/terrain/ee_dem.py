...
def export_pnw_1km_dem(bucket="wudr", description="dem_pnw_1km"):
    """Export a seamless SRTM DEM aggregated to the PNW 1 km grid.

    The output is pixel-aligned to prep.pnw_1km_grid (EPSG:5070, 1000 m,
    origin -2349000, 3214000, 1758 x 1076 pixels).  SRTM 30 m is aggregated
    via ee.Reducer.mean() to 1 km, so the result is a single continuous
    raster with no MGRS tile seams.
    """
    # PNW 1 km grid constants (from prep/pnw_1km_grid.py)
    crs = "EPSG:5070"
    scale = 1000
    x_origin = -2349000.0
    y_origin = 3214000.0
    ...
    crs_transform = [scale, 0, x_origin, 0, -scale, y_origin]

    # Bounding geometry in EPSG:5070
    x_min, y_max = x_origin, y_origin
    x_max = x_min + width * scale
    y_min = y_max - height * scale
    region = ee.Geometry.Rectangle([x_min, y_min, x_max, y_max], crs, geodesic=False)
    ...
    task = ee.batch.Export.image.toCloudStorage(
        ...
        bucket=bucket,
        fileNamePrefix=f"dem_5070/{description}",
        crs=crs,
    ...
    task.start()
    print(f"Started EE task: {description}")
    print(f"  Grid: {width} x {height} @ {scale} m, EPSG:5070")
...
if __name__ == "__main__":
    ...
    if station_set == "madis":
        ...
        sites = os.path.join(
            d, "climate", "ghcn", "stations", "ghcn_stations_mgrs_country.csv"
        )
